...
pub async fn authenticate() -> anyhow::Result<()> {
    let keyring_access_token = Entry::new("tasked", "access-token")?;
    let keyring_refresh_token = Entry::new("tasked", "refresh-token")?;
    ...
    keyring_access_token.set_secret(token_response.access_token().secret().as_bytes())?;
